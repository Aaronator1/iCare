...
function ExecutableHelper() {
    ...
    var getValAction = function(selector) {
        ...
        return ExecutableHelper.IsNullOrEmpty(something) && $(selector).is(':not(input,select,textarea)')
            ? $.trim($(selector).html())
            : something;
    };
    ...
    this.TrySetValue = function(element, val) {
        ...
        if ($(element).is('[type=hidden]') &&
            $(element).length == 2 &&
            $.byName($(element).prop('name')).length == 2) {
            this.TrySetValue(element.get(0));
            this.TrySetValue(element.get(1));
        }
